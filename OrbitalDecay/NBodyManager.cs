        ...
        /*
                public static void ManageOrbitalPredictons()
                {
                    ...
                    for (int i = 0; i < FlightGlobals.Bodies.Count; i++)
                    {
                        ...
                        {
                            if (FlightGlobals.Bodies[i] != Sun.Instance.sun)
                            {
                                ManageOrbitalPredictionsBody(FlightGlobals.Bodies[i]);
                                ManageBody(FlightGlobals.Bodies[i]);
                            }
                        }
                    }
                    ...
                    {
                        if (!VesselData.FetchStationKeeping(FlightGlobals.Vessels[j]) && FlightGlobals.Vessels[j].vesselType == VesselType.Station) // debugging
                        {
                           // ManageOrbitPredictionsVessel(FlightGlobals.Vessels[j]); For the moment
                        }
                    }
                }
